...
public:double average(vector <int> numList) {	
		double dev=0;
		vector<double> zk;
		for(int i=0;i<(1<<numList.sz);i++){
			vector<int> tmp;
			for(int j=0;j<numList.sz;j++){
				if((i&(1<<j))!=0)
					tmp.push_back(numList[j]);				
			}
			if(tmp.sz==0)continue;
			double sm=accumulate(all(tmp),0);
			sm=sm/(double)tmp.sz;
			zk.push_back(sm);
		}
		for(int i=0;i<zk.sz;i++) dev+=zk[i];
		dev=dev/(double)zk.sz;
		return dev;		
	}
	//Powered by [Ziklon]
};
