...
public:int makePalindrome(vector <int> seq) {	
			...
			while(!Q.empty()){
				node nd=Q.front();
				Q.pop();
				if(isPal(nd.ind))return nd.cost;
				vector<int> aux=nd.ind;
				for(int i=0;i+1<aux.sz;i++){
					aux[i]=aux[i]+aux[i+1];
					if(M[aux]==0){
						Q.push(node(nd.cost+1,aux));
						M[aux]=1;
					}
					aux[i]-=aux[i+1];
					aux[i+1]+=aux[i];
