...
string replace(string s, string x, string patron){
	int p,n=x.sz;	
	while(1){
		p=s.find(x);
		if(p==-1)break;
		s.replace(p,n,patron);				
	}
	return s;
}
class LinearPolyominoCovering {
public:string findCovering(string region) {	
		region=replace(region,"XXXX","AAAA");
		region=replace(region,"XX","BB");
		if(region.find("X")!=-1)return "impossible";
		return region;		
	}
	//Powered by [Ziklon]
};
