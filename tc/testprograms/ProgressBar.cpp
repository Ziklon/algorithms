...
class ProgressBar {
public:string showProgress(vector <int> taskTimes, int tasks) {	
		double sumTask=accumulate(all(taskTimes),0);
		double comple=0;
		int nv=0;
		string dev=string(20,'.');
		for(int i=0;i<tasks;i++)comple+=taskTimes[i];
		nv=(int)((comple/sumTask)*20.0);
		cout<<nv<<endl;
		for(int i=0;i<nv;i++)dev[i]='#';
		return dev;
