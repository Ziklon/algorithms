...
public:int getMaximum(int numRed, int numBlue, int onlyRed, int onlyBlue, int bothColors) {	
		int dev=numRed*onlyRed + numBlue*onlyBlue;
		int tot=numRed+numBlue;
		int aux=bothColors*(min(numRed,numBlue)*2);
		if(numRed>numBlue)
			aux+=(numRed-numBlue)*onlyRed;
		else
			aux+=(numBlue-numRed)*onlyBlue;
		return max(aux,dev); 		
	}
	//Powered by [Ziklon]
};
