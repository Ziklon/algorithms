...
int u[55];
class TomekPhone {
public:int minKeystrokes(vector <int> fs, vector <int> keys) {	
		int ans=0;
		sort(rall(fs));
		int sum=0;
		for(int i=0;i<55;i++)u[i]=0;
		
		for(int i=0;i<keys.sz;i++)sum+=keys[i];
		if(sum<fs.sz)return -1;
		int index=0;
		for(int i=0;i<fs.sz;i++){
			while(u[index]==keys[index])index=(index+1)%keys.sz;
			ans+=((u[index]+1)*fs[i]);
			u[index]++;
			index=(index+1)%keys.sz;					
		}
