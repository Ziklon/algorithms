...
public:int getMax(vector <string> data) {	
		int dev=0;
		int n=min((int)data.sz,(int)data[0].sz);
		for(int i=n;i>=0;i--){
			for(int j=0;j+i<data.sz;j++){
				for(int k=0;k+i<data[j].sz;k++){
					if(data[j][k]==data[j][k+i] && data[j+i][k]==data[j+i][k+i] && data[j][k]==data[j+i][k]){
						dev=max(dev,(i+1)*(i+1));
					}
				}
			}
