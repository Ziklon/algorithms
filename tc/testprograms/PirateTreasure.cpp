...
public:double getDistance(vector <int> steps, vector <string> directions) {	
		double x=0,y=0;
		for(int i=0;i<steps.sz;i++){
			double a=steps[i]/sqrt(2);
			if(directions[i]=="NORTH"){
				...
				x+=a;
				y-=a;			
			}else if(directions[i]=="SOUTHWEST"){
				x-=a;
				y-=a;
			}
		}		
		return sqrt(aa(x)+aa(y));
...
int main() {
	...
	int t0[] = {34,48,34};
			p0.assign(t0, t0 + sizeof(t0) / sizeof(t0[0]));
	string t1[] = {"NORTH","SOUTHWEST","EAST"};
			p1.assign(t1, t1 + sizeof(t1) / sizeof(t1[0]));
	...
	int t0[] = {2,2,2,1};
			p0.assign(t0, t0 + sizeof(t0) / sizeof(t0[0]));
	string t1[] = {"NORTH","NORTH","NORTH","NORTHWEST"};
			p1.assign(t1, t1 + sizeof(t1) / sizeof(t1[0]));
