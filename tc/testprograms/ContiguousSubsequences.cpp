...
public:vector <int> findMaxAverage(vector <int> a, int K) {	
		vector<int> dev;
		int cant=0;
		double tmp=0;
		for(int i=0;i<a.size();i++){
			for(int j=K;j+i<=a.sz;j++){
				int var=0;
				int p=i,m;
				for(m=0;m<j && p<a.sz;m++,p++)
					var+=a[p];
				double lisa=(double)var/j;
				
				if((lisa==tmp && cant<j) || lisa>tmp){
					cant=j;
					tmp=lisa;
					dev.clear();
					dev.push_back(i);
...
int main() {
	...
	{
	// ----- test 3 -----
	int t0[] = {381,921,513,492,135,802,91,519};
			p0.assign(t0, t0 + sizeof(t0) / sizeof(t0[0]));
	p1 = 1;
	int t2[] = {1,1};
			p2.assign(t2, t2 + sizeof(t2) / sizeof(t2[0]));
